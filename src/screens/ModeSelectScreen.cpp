...
void ModeSelectScreen::Render(core::AppContext& ctx) {
    ...
    // Plynne pojawianie/znikanie (~2.5 s cykl) — wspolne dla obu napisow.
    const float pulse =
    ...
    const SDL_Color prompt_col{255, 215, 0, 255};

    const char* kBoxer = "Press Start Boxer";
    const char* kKicker = "Press Start Kicker";
    const int text_h = r.MeasureText(kBoxer, ui::FontSize::Large).y;
    const int gap = lay.PH(0.022f);

    // Male okienko wideo 9:16: napis Boxer nad, Kicker pod.
    const int vid_h = lay.PH(0.42f);
    const int vid_w = (vid_h * 9) / 16;
    const int boxer_y = header_h + lay.PH(0.02f);
    const int vid_y = boxer_y + text_h + gap;
    const SDL_Rect video{cx - vid_w / 2, vid_y, vid_w, vid_h};

    r.DrawText(kBoxer, ui::FontSize::Large, prompt_col, cx, boxer_y, true, alpha, 1.0f, 3);
    ...
    const int kicker_y = video.y + video.h + gap;
    r.DrawText(kKicker, ui::FontSize::Large, prompt_col, cx, kicker_y, true, alpha, 1.0f, 3);
