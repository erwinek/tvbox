...
constexpr int kBottomMargin = 58;
constexpr int kGap = 12;
constexpr int kLeftMargin = 40;
constexpr int kPillPad = 20;
constexpr int kPillH = 50;
...
void PillAt(ui::Renderer& r, int x, int bottom_y, const std::string& label, const std::string& value,
    ...
    const int pill_w = sz.x + 2 * kPillPad;
    const int y = bottom_y - kPillH;

    r.Panel(SDL_Rect{x, y, pill_w, kPillH}, 16, SDL_Color{16, 18, 38, 210},
            SDL_Color{80, 90, 160, 170});

    const int ty = y + (kPillH - sz.y) / 2;
    SDL_Point lw = r.MeasureText(label, ui::FontSize::Small);
    r.DrawText(label, ui::FontSize::Small, SDL_Color{150, 155, 180, 255}, x + kPillPad, ty, false);
    r.DrawText(value, ui::FontSize::Small, value_col, x + kPillPad + lw.x, ty, false);
}

}  // namespace

void RenderHud(ui::Renderer& r, int credits, const std::vector<ui::ScoreEntry>& leaderboard) {
    const int bottom_y = r.height() - kBottomMargin;
    int x = kLeftMargin;
    ...
    if (!leaderboard.empty()) {
        ...
        x += PillWidth(r, "REKORD  ", record_val) + kGap;
    }
