...
void LeaderboardWidget::Render(ui::Renderer& renderer, const std::vector<ui::ScoreEntry>& entries) {
    ...
    const int board_y = 140;
    const int board_h = static_cast<int>(entries.size()) * row_h + 80;

    renderer.FillRect(SDL_Rect{board_x, board_y, board_w, board_h}, SDL_Color{18, 18, 36, 220});
    renderer.DrawRect(SDL_Rect{board_x, board_y, board_w, board_h}, SDL_Color{60, 60, 120, 180});

    renderer.DrawText("TOP SCORES", ui::FontSize::Normal, SDL_Color{255, 215, 0, 255},
                      board_x + board_w / 2, board_y + 12, true);
    ...
    for (const auto& entry : entries) {
        const int row_y = board_y + 64 + line * row_h;
